...
/// damage". Fires from OnTakeDamage, which equipment resolves ahead of every carried status (see
/// Character.ActiveStatuses), so this reacts to an incoming hit before the carrier's own Dodge, Parry,
/// Shield or Block gets a say - the same "equipment resolves first" trade FlatDamageStatus's
/// incomingReduction already makes.
///
...
public class ThornsStatus : StatusEffect
{
    ...
    public ThornsStatus(int amount) : base(StatusType.None, 1)
    {
        this.amount = amount;
    }
    ...
    {
        if (info.consumeCharges && amount > 0 && info.attacker != null)
        {
            info.attacker.TakeUnblockableDamage(amount);
        }
