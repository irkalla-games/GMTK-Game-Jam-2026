...
2. **Measure the artifact on disk; do not assume either verdict.** `git diff` the prefab/asset/scene and
   ...
   thumbnail, and not the source you think you wrote. Then **look at it**: a Play Mode screenshot
   through the live Editor (see [Driving the live Editor](#driving-the-live-editor)) is what catches a
   neighbour overlapping — and check every state that changes the layout (party size 2 *and* 4, a
   difficulty row that only shows once earned).
...
5. **Force the layout to settle before saving**, and **prune children you no longer build.** The
   ...
   archived](#editor-tools-what-is-live-what-is-archived). A UI change today goes through the live
   Editor (an `eval_file` snippet — see [Driving the live Editor](#driving-the-live-editor)) or a hand
   edit in the Inspector, so settle the layout with `LayoutRebuilder.ForceRebuildLayoutImmediate`
   yourself, and remove any child object you no longer build by hand, the same way `SharpSkin` used to.
...
which is gitignored and regenerated by Unity, so if it is missing or stale, focus the Editor once. With
the Editor open, its own compile is the stronger check: `recompile`, then `console_status` (below).

**While the Editor is open, never hand-edit `.unity` or `.prefab` YAML** — Unity holds them in memory
and overwrites on its next save. Go through the live Editor (next section) or an Editor menu command.

## Driving the live Editor

The `unity` CLI (`%LOCALAPPDATA%\Unity\bin\unity.exe`) talks to the open Editor through the
`com.unity.pipeline` package in `Packages/manifest.json`. That is how to run C#, enter Play Mode, take
screenshots, run tests and edit scenes without asking anyone to click anything. The Editor must be open
on this project; after a manifest change it needs focus once to import the package.

```
unity status --json                           # an instance in state "ready" = connected
unity command --detail compact                # every command this Editor exposes (~150)
unity command eval_file <snippet.cs>          # run C# in the Editor - see below
unity command editor_play | editor_stop       # then poll editor_status until playMode matches
unity command capture_game_view --source screen --width 1280 --height 720
unity command menu --path "Tools/Build Mode/Playable"
unity command recompile                       # then poll recompile_status
unity command console_status                  # compile-failure flag + error/warning counts
unity command list_tests | run_tests          # then poll test_status
```

Add `--json --no-banner --project-path <repo>` when parsing. Traps, all hit for real:

- **Use `eval_file`, never `eval`, from PowerShell.** Windows PowerShell 5.1 mangles the quotes in a
  multi-line argument to a native exe, and `eval` fails with exit code 2 and no message. A snippet is a
  method body that `return`s a value, sees every project type (`BuildMode`, `GameSettings`) and all of
  `UnityEditor`, and lives outside `Assets/` so writing it imports nothing.
- **`capture_game_view` needs `--source screen`** — the default renders a camera and misses every
  Screen Space - Overlay canvas, which is all of this game's UI. **Never pass `save_path`**: it is
  relative to `Assets/`, so the PNG is imported into the project. Decode the inline `base64` instead.
- **`recompile_status` returns its result as a JSON string**, not an object; parse it before reading
  `status`, or a polling loop never sees `completed`. Files added from outside need
  `AssetDatabase.Refresh()` (via `eval_file`) before a recompile will see them.
- **Edit through `SerializedObject`**, then `EditorSceneManager.SaveScene` for scenes, or
  `PrefabUtility.LoadPrefabContents` → `SaveAsPrefabAsset` for prefabs. That records prefab-instance
  overrides the way the Inspector does, where a direct property write on an instance may not.
- **Objects under an inactive parent are invisible to `GameObject.Find`** — `CharacterSelectPanel`'s
  whole backdrop is inactive in the scene. Start from
  `FindFirstObjectByType<T>(FindObjectsInactive.Include)` and walk with `transform.Find`.
- **PlayerPrefs are the user's real ones.** An `eval_file` in Play Mode that sets a key to test a path
  must restore the original value, or delete the key if it did not exist.
- **Parameter names come from the Editor, not from guessing** — `delete_asset` takes `--asset`, not
...
Project Settings → Pipeline → Runtime's `enableInBuilds` stays off regardless.
...
**Debug tools exist only in Build Mode Debug.** `Tools/Build Mode/Debug` and `/Playable`
(`BuildModeMenu`) add or remove the `DEBUG_TOOLS` define *and* flip Development Build, both on the active
build profile, then recompile. `BuildMode.DebugTools` is the compiled answer, and there are two gates on
it. `GameSettings.DebugRunEnabled` is `DebugTools && the saved toggle`, and is what the testbed,
`DebugPanel` (backquote), the pause menu's hint and the Settings row (`MenuSettingsPanel.debugRunRow`)
ask — so all of them vanish in Playable **whatever PlayerPrefs says**, which matters because a web
build's PlayerPrefs is browser storage a player can edit. A debug affordance on an ordinary screen asks
`BuildMode.DebugTools` directly instead — the party-size buttons on character select, because a Debug
Run skips that screen entirely. Never read the pref directly. The mode is committed with the profile —
**leave the repo in Debug**, switch to Playable to build for upload, and switch back. `DebugTools` is a
property, not a const, because `const false && x` is warning CS0429 in every Playable compile.

**The party size the game is balanced for is `CharacterRoster.PlayablePartySize` (2), not Min/Max.** A
Playable build plays at exactly that size with the size buttons hidden and nothing else moved; Debug
offers `MinPartySize..MaxPartySize` (2–4) and opens at the playable size. To move the game to 3, change
that one field on `DefaultRoster` — and note the tutorial hands over a party authored separately on
`MainMenu.tutorialFollowOnParty`, which stays Knight + Cleric until someone edits it.
