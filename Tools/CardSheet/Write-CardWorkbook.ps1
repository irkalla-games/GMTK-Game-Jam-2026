...
function Add-ReadmeSheet {
    ...
    $lines = @(
        ...
        @('', ''),
        @('One button does everything: in Unity, Tools > Sync With Sheet.', ''),
        ...
        @('5.', 'Save and CLOSE the workbook - the sync rewrites this file, so it cannot be open in Excel.'),
        @('6.', 'In Unity: Tools > Sync With Sheet (top of the menu).'),
        @('', ''),
        @('Authoring in Unity instead', 'head'),
        @('Just make the asset', 'Create the CardData in the Inspector as normal. The next sync adds a row for it here - you never add it by hand.'),
        @('Renaming / refiling', 'Change Key to rename the .asset, or Folder to move it. Both go through AssetDatabase.MoveAsset, so the GUID survives and decks keep pointing at the card.'),
        @('Deleting', 'Deleting a row here does NOT delete the card - it is reported and the asset kept. Delete it in Unity if you mean it.'),
        @('', ''),
        ...
        @('Card text', 'Edit Description on a class tab and sync. The change is written straight to the .asset.'),
        ...
        @('If both sides changed', 'That card is left alone on BOTH sides and named in the Unity console, showing what it was, what Unity says and what the sheet says. Make them agree, or change only one, then sync again. Nothing is overwritten behind your back.'),
        @('', ''),
